...
public class LocationModule extends BaseModule {
    ...
    public LocationModule() {
        this.defaultNameColor = legacyColor(ChatFormatting.DARK_GREEN);
    ...
    @Nullable
    private CompletableFuture<@Nullable LevelChunk> chunkFuture;
    ...
    public void update(final Minecraft client) {
        ...
        // Biome
        lines.get(7)
                .value(String.valueOf(client.level
                        .registryAccess()
                        .lookupOrThrow(Registries.BIOME)
                        .getKey(client.level.getBiome(blockPos).value())));
        ...
        if (clientChunk.isEmpty()) {
            ...
            final int totalLight =
                    client.level.getChunkSource().getLightEngine().getRawBrightness(blockPos, 0);
            ...
            // Server Chunk Lights
            final LevelLightEngine lightingProvider =
                    serverWorld.getChunkSource().getLightEngine();

            final int skyLightServer =
                    lightingProvider.getLayerListener(LightLayer.SKY).getLightValue(blockPos);
            final int blockLightServer =
                    lightingProvider.getLayerListener(LightLayer.BLOCK).getLightValue(blockPos);
            ...
            for (final Heightmap.Types type : heightmapTypes) {
                ...
                if (type.sendToClient()) {
                    final String typeString = DebugEntryHeightmap.HEIGHTMAP_NAMES.get(type);
            ...
            if (serverWorld.isInsideBuildHeight(blockPos.getY())) {
                ...
                inhabitedTime =
                        Objects.requireNonNullElse(serverChunk, clientChunk).getInhabitedTime();

                final DifficultyInstance localDifficulty = new DifficultyInstance(
                        serverWorld.getDifficulty(), serverWorld.getOverworldClockTime(), inhabitedTime, moonSize);
                localDifficultyString = String.format(
                        "%.2f  " + I18n.get("text.betterf3.line.clamped") + ": %.2f",
            ...
            if (integratedServer != null) {
                final RandomSource slimeChunk = WorldgenRandom.seedSlimeChunk(
                        chunkPos.x(), chunkPos.z(), ((WorldGenLevel) serverWorld).getSeed(), 0x3ad8025fL);
                slimeChunkString = String.format(
                        "%s",
                        I18n.get(
                                (slimeChunk.nextInt(10) == 0)
                                        ? "text.betterf3.line.slime_chunk.true"
                                        : "text.betterf3.line.slime_chunk.false"));
            } else {
        ...
        lines.get(1)
                .value(String.format(
                        "%s (%s)",
                        I18n.get("text.betterf3.line." + facing.toString().toLowerCase()), facingString));
        // Rotation
        ...
        // Ticks in the day
        lines.get(9).value(client.level.getOverworldClockTime() % 24000L);
        // Days played
        lines.get(10).value(client.level.getOverworldClockTime() / 24000L);

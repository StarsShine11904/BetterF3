...
public final class Utils {
     ...
     */
    public static MutableComponent styledText(final Object string, final TextColor color) {
     ...
     */
    public static Component formattedFromString(
            final String string, final TextColor nameColor, final TextColor valueColor) {
        final String[] split = string.split(":");

        if (string.contains(":")) {
            final MutableComponent name = Utils.styledText(split[0], nameColor);
            final MutableComponent value =
                    Utils.styledText(String.join(":", Arrays.asList(split).subList(1, split.length)), valueColor);
     ...
     * @return converted string
     */
    public static String propertyToString(final Property.Value<?> propEntry) {
        final Property<?> key = propEntry.property();
        final Comparable<?> value = propEntry.value();
